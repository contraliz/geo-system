...
export function ArticlePage({ state, onDetail }: { state: AppState; onDetail: (item: Article) => void }) {
  ...
  return <div className="content-stack">
    <Toolbar search={search} setSearch={value => { setSearch(value); setPage(1) }} selects={<>
      <select aria-label={tr('Filter by article task')} value={task} onChange={e => { setTask(e.target.value); setPage(1) }}><option value="All tasks">{tr('All tasks')}</option>{tasks.map(item => <option key={item} value={item}>{tr(item)}</option>)}</select>
