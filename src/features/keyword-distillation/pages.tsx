...
export function KeywordDistillationPage({ state, onCreate, onDetail }: KeywordDistillationPageProps) {
  ...
  return <div className="content-stack">
    ...
    <div className="keyword-grid">{items.map(item => <button className="keyword-card card" key={item.id} onClick={() => onDetail(item)}><div className="keyword-top"><span className="set-icon"><WandSparkles size={17} /></span><MoreHorizontal size={17} /></div><h3>{item.name}</h3><p className="brand-line"><span className="brand-badge">{item.brand.slice(0, 1)}</span>{item.brand} · {tr('active brand')}</p><div className="keyword-stats"><div><strong>{item.questionCount}</strong><span>{tr('questions')}</span></div><div><strong>{item.prompts.length}</strong><span>{tr('sample prompts')}</span></div><span className="asset-date">{tr('Updated')} {item.date}</span></div><div className="tag-row">{item.tags.map(tag => <span className="tag" key={tag}>{tag}</span>)}</div></button>)}</div>
  </div>
}
